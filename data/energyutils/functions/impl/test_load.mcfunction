...
execute if score #itemio.major load.status matches 0 if score #itemio.minor load.status matches 0 if score #itemio.patch load.status matches 3 run scoreboard players set #energyutils.itemio load.status 1
...
execute unless score #energyutils.itemio load.status matches 1 run tellraw @a [{"translate":"energyutils.tellraw_prefix","color":"dark_red"},{"text":"Error Loading EnergyUtils, ItemIO v0.0.3+ is required.","color":"red"}]
