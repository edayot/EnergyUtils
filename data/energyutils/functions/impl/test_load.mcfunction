...
scoreboard players set #energyutils.smithed.actionbar load.status 0
...
execute if score #itemio.major load.status matches 0 if score #itemio.minor load.status matches 1 if score #itemio.patch load.status matches 0 run scoreboard players set #energyutils.itemio load.status 1
...
execute if score #smithed.actionbar.major load.status matches 0 if score #smithed.actionbar.minor load.status matches 1 if score #smithed.actionbar.patch load.status matches 0 run scoreboard players set #energyutils.smithed.actionbar load.status 1


execute if score #energyutils.simplenergy load.status matches 1 if score #energyutils.itemio load.status matches 1 if score #energyutils.smithed.crafter load.status matches 1 if score #energyutils.rx.playerdb load.status matches 1 if score #energyutils.smithed.actionbar load.status matches 1 run function energyutils:impl/load
...
execute unless score #energyutils.itemio load.status matches 1 run tellraw @a [{"translate":"energyutils.tellraw_prefix","color":"dark_red"},{"text":"Error Loading EnergyUtils, ItemIO v0.1.0 is required.","color":"red"}]
...
execute unless score #energyutils.smithed.actionbar load.status matches 1 run tellraw @a [{"translate":"energyutils.tellraw_prefix","color":"dark_red"},{"text":"Error Loading EnergyUtils, Smithed ActionBar v0.1.0 is required.","color":"red"}]
