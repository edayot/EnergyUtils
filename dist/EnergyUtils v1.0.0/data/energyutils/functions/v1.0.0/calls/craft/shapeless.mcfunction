...
execute store result score @s smithed.data if entity @s[scores={smithed.data=0}] if score count smithed.data matches 2 if data storage smithed.crafter:input {recipe: [{Count: 1b, tag: {ctc: {id: "id_filter", from: "airdox_:energyutils"}}}]} run function energyutils:v1.0.0/items/filter/repart_id
